"""The ten robustness tests + curated-resource tests, driving the pdex backend.
...
def _strat_blocks(cfg: RobustnessConfig) -> list[str]:
    """Columns to stratify A/B splits by. replicate_col is optional for pdex
    (cell-level), so include it only when set; always include block_cols."""
    cols = [cfg.replicate_col] if cfg.replicate_col else []
    return [*cols, *cfg.block_cols]
...
_BACKEND_LABEL = "pdex (Wilcoxon)"  # shown in QQ-plot subtitles
...
def test_1_within_split_null(adata: ad.AnnData, cfg: RobustnessConfig) -> TestResult:
    ...
    for pert in perts:
        ...
        if sub.n_obs < 8:
            continue
        ab = stratified_split(
            sub, group_col=cfg.pert_col, block_cols=_strat_blocks(cfg), rng=rng
        )
        sub2 = with_obs_column(sub, "_ab", ab)
        if len(set(ab)) < 2:
            continue
        try:
            de = run_pdex_de(sub2, cfg, groupby="_ab", reference="B")
...
def test_2_control_control_null(adata: ad.AnnData, cfg: RobustnessConfig) -> TestResult:
    ...
    for r in range(cfg.n_repeats):
        ...
        ab = stratified_split(
            ctrl, group_col=cfg.pert_col, block_cols=_strat_blocks(cfg), rng=rng
        )
        ...
        if len(set(labels)) < 2:
            continue
        try:
            de = run_pdex_de(sub, cfg, groupby="_pseudo", reference="ctrl_ref")
        except Exception as exc:
    ...
    if not summaries:
        return TestResult("test_2", "Control-Control Split Null", "SKIP",
                          notes="Control split could not form a valid pdex design.")
    table = pl.concat(summaries)
    frac = float(table["frac_sig"].mean())
    abs_mean = float(table["mean_lfc"].abs().mean())
    verdict, why = _verdict_from_frac_sig(frac, abs_mean)
...
def test_5_same_gene_guides(adata: ad.AnnData, cfg: RobustnessConfig) -> TestResult:
    ...
    de = run_pdex_de(sub, cfg, groupby="_guide", reference=cfg.control_pert)
...
def test_6_target_knockdown(adata: ad.AnnData, cfg: RobustnessConfig) -> TestResult:
    ...
    de = run_pdex_de(sub, cfg, groupby=cfg.pert_col, reference=cfg.control_pert)
...
def _subsample_within(
    adata: ad.AnnData, mask: np.ndarray, n: int, by: str | None, rng: np.random.Generator
) -> np.ndarray:
    """Indices: keep all cells outside ``mask``; subsample ``n`` of the masked
    cells. If ``by`` is given, stratify by it (preserve group representation);
    otherwise sample uniformly at random (pdex is cell-level — no replicate unit)."""
    keep = list(np.where(~mask)[0])
    idx = np.where(mask)[0]
    if idx.size <= n:
        return np.sort(np.concatenate([keep, idx])).astype(int)
    if by is None or by not in adata.obs.columns:
...
def _downsampling_test(
    ...
    try:
        de_full = run_pdex_de(keep, cfg, groupby=cfg.pert_col, reference=cfg.control_pert)
    except Exception as exc:
    ...
    # pdex is cell-level: each downsample level keeps `level` cells in the pool side
    # (controls for test 10, perturbed-cell-per-condition handled by stratifying on
    # pert_col for test 9). There is no pseudobulk; the meaningful depth is the
    # cell count itself (= level for controls; ~level/n_perts per perturbation).
    n_pert = max(1, len(perts))
    rows = []
    for level in grid:
        for r in range(cfg.n_repeats):
            rng = np.random.default_rng(cfg.seed + 900 + level * 7 + r)
            # For perturbed downsampling, stratify by perturbation so each condition
            # is thinned proportionally; for controls, stratify by replicate if set.
            strat_by = cfg.pert_col if target == "perturbed" else cfg.replicate_col
            sel = _subsample_within(keep, pool_mask, level, by=strat_by, rng=rng)
            sa = keep[sel].copy()
            try:
                de = run_pdex_de(sa, cfg, groupby=cfg.pert_col, reference=cfg.control_pert)
            except Exception as exc:
    ...
    if target == "perturbed":
        # cells per condition at this level (perturbed pool split across perturbations).
        by_level = by_level.with_columns(
            (pl.col("level") / max(n_pert, 1)).round(1).alias("cells_per_condition")
        )
        depth_col = "cells_per_condition"
    else:
        # controls are a single group for pdex; the level IS the control-cell count.
        by_level = by_level.with_columns(pl.col("level").alias("cells_per_condition"))
        depth_col = "cells_per_condition"

    levels_sorted = by_level["level"].to_list()
    corr_by_level = by_level["lfc_corr_to_full_mean"].to_list()
    depth_by_level = by_level[depth_col].to_list()
    ...
    shallow = any(d is not None and d < cfg.min_cells_per_group for d in depth_by_level)
    ...
    if anti_correlated:
        note += (" ANOMALY: LFC-corr-to-full is NEGATIVE at some levels — tiny cell counts give "
                 "LFCs that anti-track the full-data estimates; do not trust LFC magnitudes there.")
    if shallow:
        note += (f" WARNING: some levels keep < {cfg.min_cells_per_group} cells per condition "
                 "(see cells_per_condition) — increase the grid lower bound.")
    return TestResult(
        name, title, verdict,
