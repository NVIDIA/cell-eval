"""Shared machinery for driving cell-eval's PyDESeq2 backend under robustness tests.
...
- ``run_pydeseq2_de``      -> thin wrapper over ``cell_eval._pydeseq2_backend.compute_pydeseq2_de``
- ``run_pipeline``         -> drives ``cell_eval.MetricsEvaluator`` with ``de_methods=["pydeseq2"]``
                             (this is exactly ``cell-eval run --de-methods pydeseq2``)
...
from cell_eval._pydeseq2_backend import compute_pydeseq2_de
...
class RobustnessConfig:
    ...
    ``adata_path``, ``pert_col``, ``control_pert`` and ``replicate_col`` are truly
    required; the rest gate which tests can run.
    """

    adata_path: str
    pert_col: str
    control_pert: str
    replicate_col: str  # PyDESeq2 pseudobulk unit; required by the backend

    counts_layer: str | None = None  # raw-count layer; None => use .X (must be raw ints)
    ...
    seed: int = 0
    num_threads: int = 1
    allow_discrete: bool = True  # PyDESeq2 reads raw counts; do not auto-normalize .X
    ...
    # Warn when a pseudobulk sample is built from fewer than this many cells.
    ...
    pydeseq2_kwargs: dict[str, Any] = field(default_factory=dict)
...
def load_adata(cfg: RobustnessConfig) -> ad.AnnData:
    ...
    _require_obs(adata, cfg.replicate_col)
...
def run_pydeseq2_de(
    ...
    *,
    ...
    """Run cell-eval's PyDESeq2 backend on ``adata``; return the canonical DE frame.

    Contrasts every non-reference level of ``groupby`` against ``reference``.
    """
    ...
    return compute_pydeseq2_de(
        ...
        pydeseq2_kwargs=cfg.pydeseq2_kwargs or None,
    )
...
def run_pipeline(
    ...
    *,
    ...
    """Run the full cell-eval run pipeline (PyDESeq2 backend) on a real/pred pair.

    This is the exact code path behind ``cell-eval run --de-methods pydeseq2``.
    ...
    evaluator = MetricsEvaluator(
        ...
        counts_layer=cfg.counts_layer,
        replicate_col=cfg.replicate_col,
        de_kwargs={"pydeseq2": cfg.pydeseq2_kwargs} if cfg.pydeseq2_kwargs else None,
    )
